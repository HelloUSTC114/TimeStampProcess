...
public:
    ...
    int Get_Time() const { if(sp) return sp->Get_Time(); else return -1;}
    int Get_Counter() const { if(sp) return sp->Get_Counter(); else return 0;}
    int Get_Layer_Number() const { if(sp) return sp->Get_Layer_Number(); else return 0;}
    ...
    int GetADC(int index) const
    {if(!sp) return 0; return sp->GetADC(index);}
    int GetADC(int boardIndex, int index) const
    {if(!sp) return 0; return sp->GetADC(boardIndex, index);}
